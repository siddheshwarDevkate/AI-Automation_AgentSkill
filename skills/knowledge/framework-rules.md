...
- **EX-09:** The environment is verified **before** generation, by the Preflight Gate in agent/agent.md — runner, browsers, application reachability, and that the target project compiles. Generation does not begin against a target that cannot run tests.
...
- **EX-11:** Delivery is blocked until `execution-report.md` contains a per-browser result for every Test Case ID — a mechanical check of the file's contents, not a self-attested checkbox (see agent/agent.md's Delivery Gate). Generating the files is not the deliverable. The sole exception is a user-approved Preflight override, which must ship labelled "Generated, NOT Verified."
