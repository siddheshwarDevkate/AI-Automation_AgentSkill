...
Load the Knowledge files listed for Skill 06 in agent/agent.md's **Skill Dependency Matrix** — that table is the single source of truth and is deliberately not restated here. This Skill uses no Template; output format is governed directly by skills/knowledge/output-structure.md's Test Data Output section.
...
**The one sanctioned modification** is the per-run uniqueness suffix described in skills/knowledge/test-data-lifecycle.md's Unique Data Generation section, applied only to uniqueness-constrained values on state-creating test cases. The supplied value remains the base; nothing else about it changes. Every other value is written exactly as supplied.

## Test Data Lifecycle (Critical)
Values alone are not enough — data that works on the first run must still work on the tenth, and under parallel workers. Apply skills/knowledge/test-data-lifecycle.md in full:
