---
name: skill-02-application-analysis
...
Load the Knowledge files listed for Skill 02 in .claude/agent/agent.md's **Skill Dependency Matrix** — that table is the single source of truth and is deliberately not restated here. This Skill uses no Template.

## Inputs
Required: Test Case Model (from Skill 01), Application URL, login credentials, and the **Login Recipe** — all three from the Intake Gate, recorded in `execution-state.md` — plus a browser automation session.
Optional: existing framework.

**Authenticate by following the Login Recipe, not by improvising.** The user described the exact ordered steps — every dropdown, checkbox, radio button, intermediate screen, and MFA handling — and Skill 00 already built the authentication fixture from them. Log in the same way here, so what this Skill observes matches what the generated suite will actually do. If the real application requires a step the Recipe omits, that is a discrepancy to report to the Agent and fix in the Recipe and the fixture together — never a step to silently perform here and leave out of the framework.
...
A page qualifies for the inventory only if it is a distinct navigable screen — normally its own URL/route, or a full-screen destination reached via navigation. Apply the Page Identity Test from .claude/skills/knowledge/framework-architecture.md.
