...
## Output Organization
Return **generated** files grouped by folder, in order: `pages/` (one file per Page Inventory entry) → `test-data/` → `tests/` → root-level reports (`traceability-report.md`, `execution-report.md`, `execution-state.md`, `README.md`). Maintain this order for every generation.

## Page Output
Exactly one Page Object per Page Inventory entry, each extending the project's own base class (never one per UI pattern/component — see .claude/skills/knowledge/framework-architecture.md's Page vs Component distinction): private locators, constructor, navigation methods, action methods, verification methods, compound business methods. Never combine unrelated pages into one file, and never split one page's patterns into several files.

## Test Output
One Spec file per module (a Test Case Model grouping, independent from Page Inventory pages — see .claude/skills/knowledge/framework-architecture.md's Module vs Page distinction): positive tests, negative tests, boundary tests (when applicable), edge cases (when applicable). Tests validate business behaviour only, importing Page Object methods and `testData.ts` constants — never inline literals or direct locator assertions.
