...
## Input Location
Read the Test Case file from `.claude/input/` (e.g. `TestCases.xlsx`, `TestCases.csv`). If multiple candidate files exist there, ask the user which one to use rather than guessing. If the file exists but contains zero data rows, treat this as a Critical Failure — request a populated file before continuing. Never generate a framework from an empty or fabricated Test Case Model.
