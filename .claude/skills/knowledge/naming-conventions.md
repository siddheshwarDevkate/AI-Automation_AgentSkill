...
## File Naming
- **NC-101 Page Object Files:** `<PageName>Page.ts` — e.g. `LoginPage.ts`, `DashboardPage.ts`, `SearchPage.ts`, `ProfilePage.ts`. `<PageName>` must come from a Page Inventory entry (.claude/skills/knowledge/framework-architecture.md's Page Identity Test) — never from a UI pattern or feature name. `DataTablePage.ts`, `ColumnSelectionPage.ts`, and `FreeTextPage.ts` are not valid names; those belong on the Page Object of whatever real page they appear on.
...
- **NC-104 Fixture Files:** `<name>.fixture.ts` — e.g. `auth.fixture.ts`, `apiClient.fixture.ts`. Export shared setup/teardown via `test.extend()`; see .claude/skills/knowledge/framework-architecture.md's Reusable Logic Placement section.
